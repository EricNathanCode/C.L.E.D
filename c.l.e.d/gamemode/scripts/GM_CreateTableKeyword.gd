...
func setup(data: Dictionary) -> void:
	...
	var cols:    Array  = data.get("columns", [])  # Array of [col_name, col_type]

	for ch in $SQLTerminal/SQLBlock.get_children():
		ch.queue_free()

	# First line: CREATE _____ table_name (
	var row1 := HBoxContainer.new()
	row1.add_theme_constant_override("separation", 4)
	$SQLTerminal/SQLBlock.add_child(row1)
	...
	_style_input(_blank)
	row1.add_child(_blank)

	var post := Label.new()
	post.text = "  " + table + " ("
	_style_code_label(post)
	row1.add_child(post)
...
func _on_execute() -> void:
	...
	if val.to_upper() == _answer.to_upper():
		...
		_fill_result($ResultBox, _step_data.get("result_msg",
			"Table created! The CREATE TABLE statement defines the structure of your table."))
	else:
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not correct. The keyword after CREATE for a new table is TABLE.")

...
func setup(data: Dictionary) -> void:
	...
	var view_name:   String = data.get("view_name", "vw_result")
	var select_cols: String = data.get("select_cols", "*")
	var table:       String = data.get("table", "table_name")
	var condition:   String = data.get("condition", "")

	for ch in $SQLTerminal/SQLBlock.get_children(): ch.queue_free()

	# Line 1: CREATE [BLANK] view_name AS
	...
	_blank.custom_minimum_size = Vector2(80, 0)
	_blank.max_length = 10
	_blank.text_submitted.connect(func(_t): _on_execute())
	_style_input(_blank)
	row.add_child(_blank)

	var suf := Label.new()
	suf.text = "  " + view_name + "  AS"
	_style_code_label(suf)
	row.add_child(suf)

	_add_code_line("SELECT " + select_cols)
	_add_code_line("FROM " + table + (";  " if condition.is_empty() else ""))
	if not condition.is_empty():
		_add_code_line("WHERE " + condition + ";")
...
func _on_execute() -> void:
	...
	if val.is_empty():
		_fill_error($ResultBox, "Type the keyword after CREATE.")
		return
	if val.to_upper() == _answer.to_upper():
		_blank.text = _answer
		_fill_result($ResultBox, [], [], _step_data.get("result_msg", "View created! You can now SELECT from it like a table."))
	else:
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not correct. The keyword is VIEW.")

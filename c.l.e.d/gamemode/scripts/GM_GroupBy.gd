...
func _ready() -> void:
	...
	_wire_grow($SQLTerminal/SQLBlock/Line3/Blank1)
...
func setup(data: Dictionary) -> void:
	...
	$SQLTerminal/SQLBlock/Line1.text       = "SELECT " + data.get("column", "[column]") + ", COUNT(*)"
	$SQLTerminal/SQLBlock/Line2.text       = "FROM " + data.get("table", "[table]")
...
func _on_execute() -> void:
	var inp: LineEdit = $SQLTerminal/SQLBlock/Line3/Blank1
	var val: String = inp.text.strip_edges()
	if val.is_empty(): _fill_error($ResultBox, "Please type the column name to group by."); return
	if val.to_lower() == _answer.to_lower():
		...
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not the right column. Check the context.")

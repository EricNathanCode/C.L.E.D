...
func _ready() -> void:
	...
	_style_input($SQLTerminal/QueryLine/IdInput)
	_wire_grow($SQLTerminal/QueryLine/IdInput)
...
func setup(data: Dictionary) -> void:
	...
	$SQLTerminal/QueryLine/IdInput.text  = ""
	...
	$SQLTerminal/QueryLine/IdInput.grab_focus()

func _on_execute() -> void:
	var inp: LineEdit = $SQLTerminal/QueryLine/IdInput
	var val: String = inp.text.strip_edges()
	if val.is_empty():     _show_error("Please type an id number from the table."); return
	if not val.is_valid_int(): _show_error("id must be a number. Try again."); return
	if int(val) == _correct_id:
		_show_result(["id","selected option"],
...
func _show_result(headers: Array, rows: Array, msg: String) -> void:
	...
	add_child(c);  move_child(c, $ButtonRow.get_index())
...
func _show_error(msg: String) -> void:
	...
	c.add_child(lbl); add_child(c); move_child(c, $ButtonRow.get_index())

...
func setup(data: Dictionary) -> void:
	_step_data = data
	_answer    = data.get("answer", "REFERENCES")
	...
	var table:     String = data.get("table", "orders")
	var fk_col:    String = data.get("fk_col", "customer_id")
	var ref_table: String = data.get("ref_table", "customers")

	for ch in $SQLTerminal/SQLBlock.get_children(): ch.queue_free()

	_add_code_line("CREATE TABLE " + table + " (")
	_add_code_line("    id        INT   PRIMARY KEY,")
	_add_code_line("    " + fk_col + "  INT,")

	# Line with blank: "    FOREIGN KEY (fk_col) [BLANK] ref_table(id)"
	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 4)
	$SQLTerminal/SQLBlock.add_child(row)

	var pre := Label.new()
	pre.text = "    FOREIGN KEY (" + fk_col + ")  "
	_style_code_label(pre)
	row.add_child(pre)

	_blank = LineEdit.new()
	_blank.placeholder_text = "keyword"
	_blank.custom_minimum_size = Vector2(120, 0)
	_blank.max_length = 15
	_blank.text_submitted.connect(func(_t): _on_execute())
	_style_input(_blank)
	row.add_child(_blank)

	var suf := Label.new()
	suf.text = "  " + ref_table + "(id)"
	_style_code_label(suf)
	row.add_child(suf)
...
func _on_execute() -> void:
	...
	if val.is_empty():
		_fill_error($ResultBox, "Type the keyword that links to the referenced table.")
		return
	if val.to_upper() == _answer.to_upper():
		_blank.text = _answer
		_fill_result($ResultBox, [], [], _step_data.get("result_msg", "FOREIGN KEY created! Referential integrity is now enforced."))
	else:
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not correct. The keyword is REFERENCES.")
...
func _fill_result(c: Node, headers: Array, rows: Array, msg: String) -> void:
	...
	var ok: Label = Label.new(); ok.text = "Query executed successfully."
	ok.add_theme_color_override("font_color", Color("#4ADE80")); c.add_child(ok)
	if not headers.is_empty(): c.add_child(_build_table(headers, rows))
	if not msg.is_empty():
		var ml: Label = Label.new(); ml.text = msg
		ml.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		ml.add_theme_color_override("font_color", Color(0.80, 0.85, 0.95)); c.add_child(ml)

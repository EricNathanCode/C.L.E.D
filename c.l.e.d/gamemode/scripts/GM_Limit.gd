...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","5")
...
func _on_execute() -> void:
	...
	if val.is_empty(): _fill_error($ResultBox, "Type a number to limit the rows returned."); return
	if val == _answer:
		_blank.text = _answer
		...
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. Type the number " + _answer + ".")

...
#             result_headers, result_rows, result_msg, err_hint
# ═══════════════════════════════════════════════════════
...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","")
	$Description.text = data.get("desc","")
	$HintLabel.text = "Hint: " + data.get("hint","")
	var tbl: String = data.get("table","")
	if tbl != "":
		$TableLabel.text = "TABLE: " + tbl
	...
	# Any leading lines (before a final \n) render as their own labels above the row
	if prefix.contains("\n"):
		var lines: PackedStringArray = prefix.split("\n")
		for i in range(lines.size() - 1):
			_add_code_line(lines[i])
		prefix = lines[lines.size() - 1]

	# Any trailing lines (after a first \n) render as their own labels below the row
	var suffix_tail: PackedStringArray = []
	if suffix.contains("\n"):
		var slines: PackedStringArray = suffix.split("\n")
		suffix = slines[0]
		for i in range(1, slines.size()):
			suffix_tail.append(slines[i])

	var row := HBoxContainer.new(); row.add_theme_constant_override("separation", 4)
	row.alignment = BoxContainer.ALIGNMENT_BEGIN
	$SQLTerminal/SQLBlock.add_child(row)
	if prefix != "":
		var pre := Label.new(); pre.text = prefix; _style_code_label(pre); row.add_child(pre)
	_blank = LineEdit.new(); _blank.placeholder_text = data.get("placeholder","keyword")
	_blank.custom_minimum_size = Vector2(110, 0); _blank.max_length = data.get("max_length", 12)
	_blank.text_submitted.connect(func(_t): _on_execute()); _style_input(_blank); row.add_child(_blank)
	if suffix != "":
		var suf := Label.new(); suf.text = suffix; _style_code_label(suf); row.add_child(suf)
	for tail_line in suffix_tail:
		_add_code_line(tail_line)
...
func _on_execute() -> void:
	...
	if val.is_empty(): _fill_error($ResultBox, "Type the missing SQL keyword."); return
	if val.to_upper() == _answer.to_upper():
		...
		on_wrong.emit()
		var eh: String = _step_data.get("err_hint", "The correct keyword is " + _answer + ".")
		_fill_error($ResultBox, "'" + val + "' is not correct. " + eh)

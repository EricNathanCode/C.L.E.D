...
func setup(data: Dictionary) -> void:
	...
	$SQLBlock/Line2/WhereKeyword.text    = "WHERE " + data.get("column", "[column]") + " = '"
...
func _on_execute() -> void:
	...
	if val.is_empty():
		_show_result(false, "Please type the search value.")
		return
	if val.to_lower() == _answer.to_lower():
		inp.text = _answer
		_show_result(true, "Query executed. Record found.")
		$ContinueButton.visible = true
	else:
		_show_result(false, "'" + val + "' not found. Check the dialogue and try again.")

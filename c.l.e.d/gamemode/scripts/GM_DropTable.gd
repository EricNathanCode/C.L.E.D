...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","TABLE")
	...
	var pre := Label.new(); pre.text = "DROP  "; _style_code_label(pre); row.add_child(pre)
	...
	var suf := Label.new(); suf.text = "  " + data.get("table","table_name") + ";"; _style_code_label(suf); row.add_child(suf)
	_blank.grab_focus()

func _on_execute() -> void:
	if _blank == null: return
	var val: String = _blank.text.strip_edges()
	if val.is_empty(): _fill_error($ResultBox, "Type the keyword after DROP."); return
	if val.to_upper() == _answer.to_upper():
		_blank.text = _answer
		_fill_result($ResultBox, [], [], _step_data.get("result_msg","Table dropped successfully."))
	else:
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. After DROP, type TABLE.")

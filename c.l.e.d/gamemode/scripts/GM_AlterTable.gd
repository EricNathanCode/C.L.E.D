...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","ADD")
	...
	_add_code_line("ALTER TABLE " + data.get("table","table_name"))
	...
	_blank.custom_minimum_size = Vector2(90, 0); _blank.max_length = 10
	_blank.text_submitted.connect(func(_t): _on_execute()); _style_input(_blank); row.add_child(_blank)
	var suf := Label.new(); suf.text = "  " + data.get("new_col","col") + "  " + data.get("col_type","TEXT") + ";"; _style_code_label(suf); row.add_child(suf)
	_blank.grab_focus()

func _on_execute() -> void:
	if _blank == null: return
	var val: String = _blank.text.strip_edges()
	if val.is_empty(): _fill_error($ResultBox, "Type the keyword to add a new column."); return
	if val.to_upper() == _answer.to_upper():
		_blank.text = _answer
		_fill_result($ResultBox, [], [], _step_data.get("result_msg","Column added successfully!"))
	else:
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. The keyword to add a column is ADD.")

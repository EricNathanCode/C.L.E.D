...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","COMMIT")
	$Description.text = data.get("desc",""); $HintLabel.text = "Hint: " + data.get("hint","")
	...
	_add_code_line("BEGIN;")
	_add_code_line("    " + data.get("update_line","UPDATE table SET col = val WHERE id = 1") + ";")
	var row := HBoxContainer.new(); row.add_theme_constant_override("separation", 4)
	$SQLTerminal/SQLBlock.add_child(row)
	_blank = LineEdit.new(); _blank.placeholder_text = "keyword"
	_blank.custom_minimum_size = Vector2(110, 0); _blank.max_length = 10
	_blank.text_submitted.connect(func(_t): _on_execute()); _style_input(_blank); row.add_child(_blank)
	var suf := Label.new(); suf.text = ";"; _style_code_label(suf); row.add_child(suf)
	var note := Label.new(); note.text = "  -- save or cancel all changes above"
	note.add_theme_font_size_override("font_size", 12)
	note.add_theme_color_override("font_color", Color(0.45, 0.52, 0.60))
	$SQLTerminal/SQLBlock.add_child(note)
	_blank.grab_focus()

func _on_execute() -> void:
	if _blank == null: return
	var val: String = _blank.text.strip_edges()
	if val.is_empty(): _fill_error($ResultBox, "Type COMMIT to save or ROLLBACK to cancel."); return
	if val.to_upper() == _answer.to_upper():
		_blank.text = _answer
		_fill_result($ResultBox, [], [], _step_data.get("result_msg","Transaction committed! All changes saved permanently."))
	else:
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. Type " + _answer + " to complete the transaction.")

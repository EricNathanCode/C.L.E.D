...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","IN")
	...
	_blank.custom_minimum_size = Vector2(80, 0); _blank.max_length = 5
	_blank.text_submitted.connect(func(_t): _on_execute()); _style_input(_blank); row.add_child(_blank)
	var suf := Label.new(); suf.text = "  " + data.get("in_list","('val1','val2')") + ";"; _style_code_label(suf); row.add_child(suf)
...
func _on_execute() -> void:
	...
	if val.is_empty(): _fill_error($ResultBox, "Type the list membership keyword."); return
	if val.to_upper() == _answer.to_upper():
		...
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. The keyword is IN.")

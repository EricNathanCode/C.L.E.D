...
func setup(data: Dictionary) -> void:
	_step_data = data
	_answer    = data.get("answer", "PRIMARY KEY")
	...
	var pk_col: String = data.get("pk_col", "id")
	...
	# CREATE TABLE opening line
	_add_code_line("CREATE TABLE " + table + " (")

	for i in range(cols.size()):
		var col_name: String = cols[i][0]
		var col_type: String = cols[i][1]
		var is_last:  bool   = (i == cols.size() - 1)
		var suffix:   String = "," if not is_last else ""

		if col_name == pk_col:
			# This column gets the blank for PRIMARY KEY
			var row := HBoxContainer.new()
			row.add_theme_constant_override("separation", 4)
			$SQLTerminal/SQLBlock.add_child(row)

			var indent := Label.new()
			indent.text = "    " + col_name + "  " + col_type + "  "
...
func _on_execute() -> void:
	...
	if val.is_empty():
		_fill_error($ResultBox, "Type the constraint name in the blank.")
		return

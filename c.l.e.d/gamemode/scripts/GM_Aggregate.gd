...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","COUNT")
	...
	var suf := Label.new(); suf.text = "(" + data.get("column","col") + ")  FROM  " + data.get("table","table") + ";"; _style_code_label(suf); row.add_child(suf)
...
func _on_execute() -> void:
	...
	if val.is_empty(): _fill_error($ResultBox, "Type an aggregate function name: COUNT, SUM, or AVG."); return
	if val.to_upper() == _answer.to_upper():
		...
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. The aggregate function here is " + _answer + ".")

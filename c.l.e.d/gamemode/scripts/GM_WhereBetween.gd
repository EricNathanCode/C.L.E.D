...
func setup(data: Dictionary) -> void:
	_step_data = data; _answer = data.get("answer","BETWEEN")
	...
	var pre := Label.new(); pre.text = "WHERE " + data.get("column","col") + "  "; _style_code_label(pre); row.add_child(pre)
	...
	var suf := Label.new(); suf.text = "  " + str(data.get("low","low")) + "  AND  " + str(data.get("high","high")) + ";"; _style_code_label(suf); row.add_child(suf)
...
func _on_execute() -> void:
	...
	if val.is_empty(): _fill_error($ResultBox, "Type the keyword for range filtering."); return
	if val.to_upper() == _answer.to_upper():
		...
		on_wrong.emit(); _fill_error($ResultBox, "'" + val + "' is not correct. The range keyword is BETWEEN.")

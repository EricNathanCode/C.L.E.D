...
func setup(data: Dictionary) -> void:
	...
	# Display empty string cells as "NULL" so the player can see which rows have no value
	var headers: Array = data.get("table_headers", [])
	var rows: Array    = data.get("table_rows", [])
	var display_rows: Array = []
	for row in rows:
		var dr: Array = []
		for cell in row:
			dr.append("NULL" if str(cell).strip_edges().is_empty() else str(cell))
		display_rows.append(dr)
	_fill_table($DataTable, headers, display_rows)
...
func _on_execute() -> void:
	...
	if val.is_empty():
		_fill_error($ResultBox, "Please type NULL or NOT NULL.")
		return
	if val.to_upper() == _answer.to_upper():
		...
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not correct. Type NULL or NOT NULL.")
...
func _cell(txt: String, is_header: bool, min_w: float) -> PanelContainer:
	...
	# Highlight NULL cells in a dimmer red so players can spot them instantly
	if txt == "NULL" and not is_header:
		s.bg_color = Color(0.18, 0.07, 0.07)

...
func setup(data: Dictionary) -> void:
	...
	var index_name: String = data.get("index_name", "idx_col")
	var table:      String = data.get("table", "table_name")
	var column:     String = data.get("column", "column_name")

	for ch in $SQLTerminal/SQLBlock.get_children(): ch.queue_free()

	# Line 1: CREATE [BLANK] index_name
	...
	_blank.custom_minimum_size = Vector2(90, 0)
	...
	_add_code_line("ON " + table + "(" + column + ");")
	_blank.grab_focus()
...
func _on_execute() -> void:
	...
	if val.to_upper() == _answer.to_upper():
		_blank.text = _answer
		_fill_result($ResultBox, [], [], _step_data.get("result_msg", "Index created! Queries on this column will now run faster."))
	else:
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not correct. The keyword is INDEX.")

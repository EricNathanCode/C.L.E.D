...
func setup(data: Dictionary) -> void:
	...
	# Line 1: SELECT DISTINCT [blank] FROM table;
	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 4)
	$SQLTerminal/SQLBlock.add_child(row)
	var pre := Label.new(); pre.text = "SELECT DISTINCT  "; _style_code_label(pre); row.add_child(pre)
	_blank = LineEdit.new()
	_blank.placeholder_text = "column"
	_blank.custom_minimum_size = Vector2(140, 0)
	_blank.max_length = 30
...
func _on_execute() -> void:
	...
	if val.is_empty(): _fill_error($ResultBox, "Type the column name to select unique values from."); return
	if val.to_lower() == _answer.to_lower():
		...
		on_wrong.emit()
		_fill_error($ResultBox, "'" + val + "' is not a column in this table. Check the hint.")
